...
static void processNotification(ConsoleNotificationType type, void* parameter) {
    ...
    case ConsoleNotificationType::CodeBreak: {
        ...
        case BreakSource::PpuStep:
		  ...
		  case BreakSource::Unspecified: { // first dummy event for debugger
			  ev.pid = 1;
			  ev.tid = 1;
			  ev.ea = GetPcAddress();
			  ev.handled = true;
...
static const char* get_mesen_path() {
    qstring path(idadir(PLG_SUBDIR));
    //path.append("/mesen-s");
...
static bool register_events_callback() { // move debug events to shared
...
static bool unregister_events_callback() { // move debug events to shared
    UnregisterNotificationCallback(listener);

    return true;
}

static bool update_bpts(int* nbpts, update_bpt_info_t* bpts, int nadd, int ndel, qstring* errbuf) { // move breakpoints list to shared
    ...
    for (int j = 0; j < execs + reads + writes; ++j) {
        ...
        for (int i = 0; i < ndel; ++i) {
            ...
            if (static_cast<int32_t>(bpts[nadd + i].ea) >= typeBpts[j].startAddr && (static_cast<int32_t>(bpts[nadd + i].ea) + bpts[nadd + i].size <= typeBpts[j].endAddr)) {
                continue;
            }
        }
...
static bool write_register(thid_t tid, int regidx, const regval_t* value, qstring* errbuf) {
    ...
    case SNES_REGS::SR_A: cpuReg0 = CpuRegister::CpuRegA; break;
    case SNES_REGS::SR_X: cpuReg0 = CpuRegister::CpuRegX; break;
    case SNES_REGS::SR_Y: cpuReg0 = CpuRegister::CpuRegY; break;
    case SNES_REGS::SR_SP: cpuReg0 = CpuRegister::CpuRegSP; break;
    case SNES_REGS::SR_D: cpuReg0 = CpuRegister::CpuRegD; break;
    case SNES_REGS::SR_DBR: cpuReg0 = CpuRegister::CpuRegDBR; break;
    case SNES_REGS::SR_PC: cpuReg0 = CpuRegister::CpuRegPC; break;
    default:
        return false;
    }
...
static bool start_emulation(const char* rom) {
    qstring exe_path = idadir(nullptr);
    exe_path.append("/Mesen-S.exe");

    //launch_process_params_t launch = {};
    //launch.cb = sizeof(launch_process_params_t);
    //launch.path = exe_path.c_str();
    //launch.startdir = get_mesen_path();
    //launch.args = rom;
    //launch.in_handle = -1;
    //launch.out_handle = -1;

    //qstring errbuf;
    //launch_process(launch, &errbuf);

	 //SetDllDirectoryA(mesen_path.c_str());
    ...
    return register_events_callback();
}

static bool exit_emulation(qstring* errbuf) {
    Release();

    return unregister_events_callback();
}
...
debugger_t debugger {
    ...
    DBG_FLAG_NOHOST | DBG_FLAG_CAN_CONT_BPT | DBG_FLAG_SAFE | DBG_FLAG_FAKE_ATTACH | DBG_FLAG_NOPASSWORD | DBG_FLAG_NOPARAMETERS | DBG_FLAG_ANYSIZE_HWBPT | DBG_FLAG_DEBTHREAD,
    DBG_HAS_REQUEST_PAUSE | DBG_HAS_SET_RESUME_MODE | DBG_HAS_THREAD_SUSPEND | DBG_HAS_THREAD_CONTINUE /*| DBG_HAS_MAP_ADDRESS*/,
